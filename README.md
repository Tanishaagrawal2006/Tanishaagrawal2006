<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0d0d1a,30:1a1040,60:2d1b69,100:0f0c29&height=220&section=header&text=Tanisha%20Agrawal&fontSize=52&fontColor=ffffff&animation=fadeIn&fontAlignY=40&desc=Data%20Analyst%20%E2%80%A2%20Data%20Scientist%20%E2%80%A2%20AI%20Engineer&descAlignY=60&descSize=20" />

<br/>

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com/?font=Inter&weight=600&size=22&duration=3000&pause=900&color=D946EF&center=true&vCenter=true&width=700&lines=Data+Analyst+%26+Scientist;AI+%26+Data+Intelligence+Intern+%40+Graphura+India;PR+Lead+%40+HackData+%26+Surge;Google+Developer+Groups+-+EM+Lead" alt="Typing SVG" />
</a>

<br/><br/>

[![LinkedIn](https://img.shields.io/badge/LinkedIn-4338CA?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/tanisha-agrawal-288927341/)
[![Email](https://img.shields.io/badge/Email-7C3AED?style=for-the-badge&logo=gmail&logoColor=white)](mailto:ag.tanisha06@gmail.com)
[![LeetCode](https://img.shields.io/badge/LeetCode-1a1030?style=for-the-badge&logo=leetcode&logoColor=FFA116)](https://leetcode.com/u/Tanishaagrawal_06/)
[![GitHub](https://img.shields.io/badge/GitHub-C026D3?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Tanishaagrawal2006)

</div>

---

## About Me

I am Tanisha, a Computer Science Engineering student with a focus on Data Analysis, Machine Learning, and Applied AI.

I work across the data lifecycle, from preprocessing and feature engineering to building and evaluating classification models, and I enjoy translating that work into practical, end-to-end applications. My recent projects span fraud detection, natural language processing, and sentiment analysis, and I have led cross-functional teams in both technical and organizational settings.

Alongside my coursework and project work, I take on leadership roles in campus technical and community events, coordinating large-scale outreach and operations for hackathons and fests.

---

## Experience

**Data and AI Intelligence Intern — Graphura India Pvt. Ltd.** &nbsp;·&nbsp; `05/2026 – 07/2026`

Collaborated within a five-member team to develop AI and data-driven solutions for real-world problems, contributing to the design, implementation, testing, and deployment of Machine Learning, NLP, and rule-based systems. Led the Data Science and AI Department, coordinating forty members across eight project teams.

**Admissions On-Campus Job — Shiv Nadar Institution of Eminence** &nbsp;·&nbsp; `01/2025 – 03/2025`

Maintained and updated confidential administrative databases with high accuracy, acted as a liaison between students, parents, and staff, and strengthened documentation and data management workflows.

---

## Projects

**Job Scam Detection — Career Building Platform**
`Python` · `Machine Learning` · `NLP` · `Supabase` · `Render` &nbsp;·&nbsp; `06/2026`

A web application that analyzes job posting URLs and predicts whether an opportunity is genuine or potentially fraudulent, using a hybrid framework that combines Machine Learning, NLP, and rule-based analysis. Includes career-development modules for profile creation, skill assessment, HR review requests, and progress tracking. Built collaboratively with a five-member team covering data extraction, system architecture, feature engineering, and model validation.

**Gen Review AI — Restaurant Review Management**
`Python` · `NLP` · `Machine Learning` &nbsp;·&nbsp; `06/2026 – 07/2026`

A QR-based customer feedback system that collects restaurant experiences and generates personalized review suggestions using sentiment analysis and text processing. Integrates Google Maps redirection through restaurant-specific Map IDs so customers can post reviews directly, streamlining the feedback-to-review workflow end to end.

**Intern Resume Readiness Scorer**
`Python` · `Machine Learning` · `Data Analysis` &nbsp;·&nbsp; `05/2026`

A portfolio evaluation system that assesses internship readiness based on skills, projects, certifications, experience, and profile completeness. Combines Machine Learning predictions with keyword-based scoring to identify skill gaps and generate personalized improvement recommendations.

---

## Technical Skills

<div align="center">

**Programming and Databases**

![Python](https://img.shields.io/badge/Python-1a1030?style=for-the-badge&logo=python&logoColor=C77DFF)
![C](https://img.shields.io/badge/C-1a1030?style=for-the-badge&logo=c&logoColor=C77DFF)
![SQL](https://img.shields.io/badge/SQL-1a1030?style=for-the-badge&logo=postgresql&logoColor=C77DFF)
![MySQL](https://img.shields.io/badge/MySQL-1a1030?style=for-the-badge&logo=mysql&logoColor=C77DFF)
![HTML](https://img.shields.io/badge/HTML-1a1030?style=for-the-badge&logo=html5&logoColor=C77DFF)

**Data and AI**

![Pandas](https://img.shields.io/badge/Pandas-241046?style=for-the-badge&logo=pandas&logoColor=D946EF)
![NumPy](https://img.shields.io/badge/NumPy-241046?style=for-the-badge&logo=numpy&logoColor=D946EF)
![scikit-learn](https://img.shields.io/badge/scikit--learn-241046?style=for-the-badge&logo=scikitlearn&logoColor=D946EF)
![Machine Learning](https://img.shields.io/badge/Machine_Learning-241046?style=for-the-badge&logoColor=D946EF)
![NLP](https://img.shields.io/badge/NLP-241046?style=for-the-badge&logoColor=D946EF)
![Feature Engineering](https://img.shields.io/badge/Feature_Engineering-241046?style=for-the-badge&logoColor=D946EF)

**Cloud and Tools**

![AWS](https://img.shields.io/badge/AWS-2d0a3d?style=for-the-badge&logo=amazonaws&logoColor=F472B6)
![Linux](https://img.shields.io/badge/Linux-2d0a3d?style=for-the-badge&logo=linux&logoColor=F472B6)
![Git](https://img.shields.io/badge/Git-2d0a3d?style=for-the-badge&logo=git&logoColor=F472B6)
![GitHub](https://img.shields.io/badge/GitHub-2d0a3d?style=for-the-badge&logo=github&logoColor=F472B6)
![Supabase](https://img.shields.io/badge/Supabase-2d0a3d?style=for-the-badge&logo=supabase&logoColor=F472B6)
![Render](https://img.shields.io/badge/Render-2d0a3d?style=for-the-badge&logo=render&logoColor=F472B6)
![Excel](https://img.shields.io/badge/Excel-2d0a3d?style=for-the-badge&logo=microsoftexcel&logoColor=F472B6)

</div>

---

## Leadership and Volunteering

**PR Lead — HackData** &nbsp;·&nbsp; `12/2025 – 01/2026`

Led public relations and outreach for a multi-event technical competition spanning hackathon, competitive programming, and CTF tracks, driving participation from over two hundred students across more than seventy colleges.

**PR Lead — Surge, Annual Sports Fest** &nbsp;·&nbsp; `07/2025 – 04/2026`

Led a thirty-five member public relations team, managing task delegation, formal communication, and on-ground coordination for an event with more than 2,200 participants from over one hundred colleges.

**EM Lead — Google Developer Groups** &nbsp;·&nbsp; `06/2025 – 05/2026`

Supported planning and execution of Google Developer Groups events, coordinating logistics and cross-functional teams to improve participant experience.

---

## Education

**B.Tech, Computer Science Engineering**
Shiv Nadar Institution of Eminence &nbsp;·&nbsp; `08/2024 – Present` &nbsp;·&nbsp; CGPA: 8.02

**Senior Secondary Education**
Sant Gyaneshwar Model School, Delhi &nbsp;·&nbsp; `2022 – 2024` &nbsp;·&nbsp; 87%

---

## GitHub Analytics

<div align="center">

<p align="center">
  <img height="165" src="https://github-readme-stats-one-bice.vercel.app/api?username=Tanishaagrawal2006&show_icons=true&hide_border=true&include_all_commits=true&count_private=true&title_color=D946EF&icon_color=C77DFF&text_color=E0AAFF&bg_color=0f0c29" />
  <img height="165" src="https://github-readme-stats-one-bice.vercel.app/api/top-langs/?username=Tanishaagrawal2006&layout=compact&hide_border=true&title_color=D946EF&text_color=E0AAFF&bg_color=0f0c29" />
</p>

<br/><br/>

<p align="center">
  <img
    src="https://github-readme-streak-stats-eight.vercel.app?user=Tanishaagrawal2006&background=0f0c29&border=0f0c29&ring=D946EF&fire=F472B6&currStreakLabel=D946EF&sideLabels=E0AAFF&currStreakNum=FFFFFF&sideNums=FFFFFF&dates=C77DFF"
    alt="GitHub Streak"
  />
</p>

<br/><br/>

<img width="100%" src="https://github-readme-activity-graph.vercel.app/graph?username=Tanishaagrawal2006&hide_border=true&bg_color=0f0c29&color=C77DFF&line=D946EF&point=F472B6&area=true&area_color=7c3aed&title_color=D946EF" alt="Activity Graph" />

</div>

---

<div align="center">

> *"Data tells the story. Analysis finds the meaning."*
>
> I build solutions that turn raw data and ambiguous problems into clear, actionable systems, one model, one pipeline, and one project at a time.

</div>

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&color=0:0f0c29,35:6a3093,70:c86dd7,100:ff8fd8&height=110&section=footer" />








